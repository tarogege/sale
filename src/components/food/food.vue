    ...
    <div v-show="showFood" class="food-wrapper" ref="food">
      <div class="food-content">
        ...
        <div class="content">
          ...
          <p class="desc"><span v-if="food.sellCount" class="count">月售{{food.sellCount}}</span><span v-if="food.rating"
                                                                                                     class="rating">好评率{{food.rating}}%</span>
          </p>
          <span class="price">¥{{food.price}}</span><span v-if="food.oldPrice"
                                                          class="old-price">¥{{food.oldPrice}}</span>
          <a @click.stop.prevent="addFirst(food,$event)" class="join-car"
             v-show="!food.count||food.count === 0">加入购物车</a>
        ...
        <div class="ratings">
          ...
          <ratingslect @select="selectRatings" @contentToggle="toggleContent" :ratings="food.ratings"
                       :catology="['全部','推荐', '吐槽']" :selectType="selectType" :onlyContent="onlyContent"></ratingslect>
        </div>
        <div class="rating-wrapper">
          <ul class="rating-list" v-show="food.ratings && food.ratings.length!== 0">
            <li v-for="(rating,index) in food.ratings" :key="index" v-show="needShow(rating.rateType,rating.text)">
              ...
              <div class="detail">
                <i class="fa"
                   :class="{'fa-thumbs-up': rating.rateType === 0,'fa-thumbs-down':rating.rateType === 1}"></i>
